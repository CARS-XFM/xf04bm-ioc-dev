...
class IonChamber(Device):
    ...
    def calculate(self):
        ...
        amp_pv    = self.get('Amp')
        PSh_pv    = 'XF:04BMB-PPS{Sh:A}Pos-Sts.RVAL'
	...
	if int(caget(PSh_pv)):
        ...
        self.put('Current',   "%10.3g" % current)
        self.put('FluxAbs',   "%9.2e" % flux_abs)
        self.put('FluxOut',   "%9.2e" % flux_trans)
...
def start_ionchamber(prefix='XF:04BM-ES:1:ION:'):
    """ save pid for later killing """
    fpid = open(PIDFILE, 'w')
    fpid.write("%d\n" % os.getpid() )
    fpid.close()

    print 'start_ionchamber with:', prefix
    ion = IonChamber(prefix=prefix)
    ion.run()

def get_lastupdate(prefix='XF:04BM-ES:1:ION:'):
